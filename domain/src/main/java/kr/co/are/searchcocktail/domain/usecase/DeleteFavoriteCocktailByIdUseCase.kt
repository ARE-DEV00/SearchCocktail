...
class DeleteFavoriteCocktailByIdUseCase @Inject constructor(
    private val databaseCocktailRepository: DatabaseCocktailRepository
) {
    suspend operator fun invoke(drinkInfoEntity: DrinkInfoEntity): Flow<ResultDomain<Boolean>> {
        return channelFlow {
            databaseCocktailRepository.deleteFavoriteDrinkInfo(drinkInfoEntity.id)
                .catch { exception ->
