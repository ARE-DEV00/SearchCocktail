...
class GetFavoriteCocktailByIdUseCase @Inject constructor(
    private val databaseCocktailRepository: DatabaseCocktailRepository
) {
    suspend operator fun invoke(id:String): Flow<ResultDomain<DrinkInfoEntity?>> {
        return channelFlow {
            databaseCocktailRepository.getFavoriteDrinkInfo(id)
                .catch { exception ->
